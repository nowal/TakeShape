...
const Luma3DModelPage = () => {
  ...
  const apiKey = 'b4266b2e-7278-4a91-920c-91e36147b0d4-c77e58e-deb2-4aec-87b3-21eea12b8170'; // Replace with your actual API key
  const fileInputRef = useRef<HTMLInputElement>(null);
  const hardcodedSlug = "34ae8ee5-99a8-43eb-97cc-c7e485284479";
  ...
  const createCapture = async () => {
    ...
    myHeaders.append("Authorization", `luma-api-key=${apiKey}`);
  ...
  const uploadVideo = async (uploadUrl: string, file: Blob) => {
    const requestOptions: RequestInit = {
      method: 'PUT',
      body: file,
      headers: {
  ...
  const checkCaptureStatus = async () => {
    ...
    try {
      const response = await fetch(`https://webapp.engineeringlumalabs.com/api/v2/capture/${hardcodedSlug}`, {
        method: 'GET',
        headers: myHeaders,
      });
      const result = await response.json();
  ...
  const downloadCapture = async () => {
    ...
    try {
      const response = await fetch(`https://webapp.engineeringlumalabs.com/api/v2/download/${hardcodedSlug}`, {
        method: 'GET',
        headers: myHeaders,
      });
  ...
  const handleCaptureAndUpload = async () => {
    if (fileInputRef.current && fileInputRef.current.files && fileInputRef.current.files.length > 0) {
      ...
      if (captureData && captureData.signedUrls && captureData.signedUrls.source && captureData.capture && captureData.capture.slug) {
        await uploadVideo(captureData.signedUrls.source, file);
  ...
  return (
    <div>
      ...
      <button onClick={() => handleCaptureAndUpload()}>Create Capture</button> {/* Adjusted onClick */}
      <button onClick={() => checkCaptureStatus()}>Check Capture Status</button> 
      <button onClick={() => downloadCapture()}>Download Capture</button>
