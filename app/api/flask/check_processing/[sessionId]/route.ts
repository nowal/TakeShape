...
const fetchWithRetry = async (url: string, options: RequestInit, retries = 3, timeout = 60000) => { // Increased retries to 3 and timeout to 60 seconds
  ...
  for (let i = 0; i <= retries; i++) {
    try {
      console.log(`Attempt ${i + 1}/${retries + 1} - Starting fetch request to check_processing...`);
...
export async function GET(
  ...
  console.log(`API route: /api/flask/check_processing/${sessionId} - GET request received`);
  
  try {
    // Forward the request to the Flask backend
    const response = await fetchWithRetry(`${FLASK_BACKEND_URL}/api/check_processing/${sessionId}`, {
      method: 'GET',
      headers: {
    ...
    console.error('Error in /api/flask/check_processing:', {
    ...
    return NextResponse.json(
      {
        error: 'Failed checking processing status in Next.js backend.',
