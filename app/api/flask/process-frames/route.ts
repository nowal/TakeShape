...
const fetchWithTimeout = async (url: string, options: RequestInit, timeout = 60000) => { // 60 seconds timeout
...
const fetchWithRetry = async (url: string, options: RequestInit, retries = 3, timeout = 60000) => {
  ...
  for (let i = 0; i <= retries; i++) {
    try {
      console.log(`Attempt ${i + 1}/${retries + 1} - Starting fetch request to process-frames...`);
...
export async function POST(request: NextRequest) {
  console.log('API route: /api/flask/process-frames - POST request received');
  ...
  try {
    ...
    console.log(`Processing frames for session: ${sessionId}`);
    
    // Get the form data from the request
    const formData = await request.formData();
    
    // Forward the request to the Flask backend
    const response = await fetchWithRetry(`${FLASK_BACKEND_URL}/api/process-frames/${sessionId}`, {
      method: 'POST',
      body: formData
    });
    ...
    console.error('Error in /api/flask/process-frames:', {
    ...
    return NextResponse.json(
      {
        error: 'Failed processing frames in Next.js backend.',
