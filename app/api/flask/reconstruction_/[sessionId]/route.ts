...
const fetchWithRetry = async (url: string, options: RequestInit, retries = 3, timeout = 60000) => { // Increased retries to 3 and timeout to 60 seconds
  ...
  for (let i = 0; i <= retries; i++) {
    try {
      console.log(`Attempt ${i + 1}/${retries + 1} - Starting fetch request to reconstruction_...`);
...
export async function GET(
  ...
  console.log(`API route: /api/flask/reconstruction_/${sessionId} - GET request received`);
  ...
  try {
    ...
    console.error('Error in /api/flask/reconstruction_:', {
