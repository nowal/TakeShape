...
const fetchWithRetry = async (url: string, options: RequestInit, retries = 3, timeout = 30000) => {
  ...
  for (let i = 0; i <= retries; i++) {
    try {
      console.log(`Attempt ${i + 1}/${retries + 1} - Starting fetch request to start-session...`);
...
export async function POST(request: NextRequest) {
  console.log('API route: /api/flask/start-session - POST request received');
  
  try {
    // Forward the request to the Flask backend
    const response = await fetchWithRetry(`${FLASK_BACKEND_URL}/api/start-session`, {
      method: 'POST',
      headers: {
    ...
    console.error('Error in /api/flask/start-session:', {
    ...
    return NextResponse.json(
      {
        error: 'Failed starting session in Next.js backend.',
