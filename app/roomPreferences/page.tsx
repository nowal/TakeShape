...
const RoomPreferences = () => {
    ...
    const attemptAccessVideo = () => {
      ...
      if (video) {
        ...
        const handleTimeUpdate = () => {
          ...
          const matchedPair = timestampPairs.find(
            (pair) =>
              currentTime >= pair.startTime &&
              (pair.endTime === undefined ||
                currentTime <= pair.endTime)
          );
    ...
    const determineCurrentTimestampOrRevertToDefaults =
      () => {
        ...
        const matchedPair = timestampPairs.find(
          (pair) =>
            currentTime >= pair.startTime &&
            (!pair.endTime || currentTime <= pair.endTime)
        );
    ...
    const fetchTimestampPairs = async () => {
      if (auth.currentUser) {
        ...
        if (!querySnapshot.empty) {
          ...
          const fetchedTimestampPairs =
            userImageDoc.timestampPairs || [];
          setTimestampPairs(
            fetchedTimestampPairs.map(
              (pair: number[], index: number) => ({
                ...pair,
                id: `pair-${index}`,
              })
            )
          ); // Adding a temporary ID
        }
      }
    };
    ...
    const fetchPaintPreferences = async () => {
      if (auth.currentUser) {
        ...
        if (docSnap.exists()) {
          ...
          const { color, finish, ceilings, trim } =
            docSnap.data();
    ...
    const unsubscribe = onAuthStateChanged(
      ...
      async (currentUser) => {
        ...
        if (currentUser) {
          ...
          const isPainterValue =
            sessionStorage.getItem('isPainter') ===
              'true' ||
            (await checkIfPainter(currentUser.uid));
  ...
  useEffect(() => {
    ...
    if (userData && userData.prices) {
      const accepted = userData.prices.find(
  ...
  const handleVideoSelection = async () => {
    ...
    if (videoRef.current) {
      ...
      const isWithinExistingPair = timestampPairs.some(
        (pair) => {
          const endTime =
            pair.endTime === undefined
              ? Infinity
      ...
      if (!querySnapshot.empty) {
        ...
        if (userImageDoc.paintPreferencesId) {
          const paintPreferencesRef = doc(
            firestore,
            'paintPreferences',
            userImageDoc.paintPreferencesId
          );
          const paintPreferencesSnap = await getDoc(
            paintPreferencesRef
          );
          if (paintPreferencesSnap.exists()) {
            const { color, finish, ceilings, trim } =
              paintPreferencesSnap.data();
            saveTimestampToFirestore(
              currentTime,
              color,
              finish,
              ceilings,
  ...
  const handleTimestampPairDelete = async (
    startTime: number
  ) => {
    setTimestampPairs((prevPairs) => {
      const updatedPairs = prevPairs.filter(
        (pair) => pair.startTime !== startTime
      );
  ...
  const saveTimestampToFirestore = async (
    startTime: number,
    color: string = defaultPaintColor,
    finish: string = defaultPaintFinish,
    dontPaintCeilings: boolean = ceilingPaint,
    dontPaintTrimAndDoors: boolean = doorsAndTrimPaint
  ) => {
    if (!auth.currentUser || !userImageRef) {
      console.error(
        'No authenticated user or user image document reference.'
      );
      return;
    }
    ...
    const newTimestampPair: TTimestampPair = {
      startTime,
      color,
      finish,
  ...
  const endRoomAndReturnToDefaults = async () => {
    if (
      ...
      console.error(
        'Missing required references or no timestamp pairs available.'
      );
      return;
    }
    if (videoRef.current) {
    ...
    const endingPairIndex = timestampPairs.findIndex(
      (pair) => pair.startTime === videoSelectionStart
    );

    // Check if the intended endTime falls within any existing timestamp pair (excluding the one being ended)
    const isWithinExistingPair = timestampPairs.some(
      (pair, index) => {
        if (index !== endingPairIndex) {
          // Exclude the pair being ended from the check
    ...
    if (endingPairIndex !== -1) {
      ...
      try {
        ...
        if (updatedDoc.exists()) {
          setTimestampPairs(
            updatedDoc.data().timestampPairs
          );
        }
      } catch (error) {
        console.error(
          'Error updating timestamp pair with endTime: ',
          error
        );
      }
    } else {
  ...
  const handleAcceptQuote = async (
    ...
    if (auth.currentUser) {
      ...
      if (!querySnapshot.empty) {
        ...
        console.error(
          'No user image document found for the current user.'
        );
          ...
          >
            ...
            <div className="flex-2 flex items-center justify-between pl-5 border-l-2 border-gray-300 gap-10">
              <p className="text-lg font-bold">
                ...
                <div className="ml-10">
                  {prices.find((price) => price.painterId)
                    ?.invoiceUrl && (
                    <a
                      href={
                        prices.find(
                          (price) => price.painterId
                        )?.invoiceUrl
                      }
                      target="_blank"
  ...
  const updateAdditionalInfo = async () => {
    ...
    try {
      ...
      console.error(
        'Error updating paint preferences or linking: ',
        error
      );
    }
  };
        ...
        <div className="dashboard-content flex flex-col items-center w-full max-w-4xl mb-16">
          ...
          <div className="new-room-container text-center mt-4 mb-4">
            ...
            <div className="tooltip-container ml-2 inline-block relative">
              ...
              >
                Click the Change Preferences button whenever
                the video enters a room or space that you
                want different preferences for.
              </span>
            </div>
          </div>
              ...
              [...timestampPairs]
                ...
                .map((pair, index) => (
                  <RoomCard
                    ...
                    userImageRef={userImageRef}
                    onDelete={() =>
                      handleTimestampPairDelete(
                        pair.startTime
                      )
                    }
                    defaultColor={
                      pair.color || defaultPaintColor
                    }
                    defaultFinish={
                      pair.finish || defaultPaintFinish
                    }
                    defaultCeilings={
                      pair.ceilings !== undefined
                        ? pair.ceilings
                        : ceilingPaint
                    }
                    defaultTrim={
                    ...
                    editable={true}
                    onClick={() =>
                      handleRoomCardClick(pair.startTime)
                    }
                  />
                ))}
          </div>
