...
const PainterCallCenter: React.FC = () => {
  ...
  useEffect(() => {
    return () => {
      ...
      if (providerAudioStreamRef.current) {
  ...
  const joinPainterRoomAudioOnly = async (
    ...
    setRoomAudioEnabled(audioEnabled);
    if (audioEnabled) {
  ...
  const createVideoRoom = async () => {
    ...
    try {
      ...
      const prepared = await prepareProviderCallSession({
        normalizedHomeownerName,
        normalizedHomeownerAddress,
        normalizedHomeownerEmail,
        normalizedHomeownerNumber: '',
      ...
      >
        ...
        >
          ...
          <p style={{ margin: 0, color: '#475569' }}>
            This call screen is restricted to signed-in provider accounts.
          </p>
        </div>
      </div>
    );
