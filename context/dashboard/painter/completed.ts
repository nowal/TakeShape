...
type TConfig = any;
export const useDashboardPainterCompleted = (config?:TConfig) => {
        ...
        const jobs = await Promise.all(userImagesSnapshot.docs.map(async (jobDoc) => {
          ...
          if (jobData.address) {
            ...
            if (lat !== undefined && lng !== undefined) {
              ...
              console.error('TJob address is missing latitude and/or longitude after geocoding:', jobData.address);
            }
          }
          return null;
        }));
        const filteredJobs = jobs.filter(job => job !== null) as TJob[];
