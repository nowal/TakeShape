...
import { TTimestampPair } from '@/types';
...
const RoomCard: FC<RoomCardProps> = ({
  ...
  const [paintCeilings, setPaintCeilings] =
    useState(defaultCeilings);
  const [paintTrimAndDoors, setPaintTrimAndDoors] =
    useState(defaultTrim);
  const [dontPaintAtAll, setDontPaintAtAll] =
    useState(false);
  const [isRoomSet, setIsRoomSet] = useState(false);
  const [editableRoomName, setEditableRoomName] =
    useState(roomName); // State variable for editable roomName

  const [
    ,
    // timestampPairs
    setTimestampPairs,
  ] = useAtom(timestampPairsAtom);
  ...
  useEffect(() => {
    const fetchTimestampPair = async () => {
      ...
      if (docSnap.exists()) {
        ...
        const specificPair = data.timestampPairs.find(
          (pair: TTimestampPair) =>
        ...
        if (specificPair) {
          ...
          setPaintCeilings(
            specificPair.paintCeilings ?? defaultCeilings
          );
          setPaintTrimAndDoors(
            specificPair.paintTrimAndDoors ?? defaultTrim
          );
  ...
  const updateTimestampPairs = async () => {
    try {
      ...
      if (docSnap.exists()) {
        const data = docSnap.data();
        let timestampPairs: TTimestampPair[] =
          data.timestampPairs || [];
        timestampPairs = timestampPairs.filter(
        ...
        console.log(
          'User image document updated successfully with new timestamp pair'
        );
      } else {
          ...
          <>
            <div className="flex items-center gap-2 mb-2 flex-wrap">
              <input
                ...
                value={color}
                onChange={(e) =>
                  editable ? setColor(e.target.value) : null
                }
              ...
              <div
                className="flex-grow flex-shrink min-w-0 flex-wrap"
                style={{ maxWidth: '35%' }}
              >
                {' '}
                {/* This div will hold checkboxes and allow for some flex growth/shrink */}
                <label className="flex items-center gap-1 whitespace-nowrap mr-2">
                  <input
                    ...
                    onChange={(e) =>
                      editable
                        ? setPaintCeilings(e.target.checked)
                        : null
                    }
                    disabled={!editable}
                  />
          ...
          <div className="flex justify-between items-center mb-2">
            <label className="flex items-center gap-1">
              <input
                ...
                onChange={(e) =>
                  setDontPaintAtAll(e.target.checked)
                }
              />
              Don&apos;t paint at all
            </label>
            <button
              type="submit"
              className="btn button-color hover:bg-green-900 text-white rounded"
            >
